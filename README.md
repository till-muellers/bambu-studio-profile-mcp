...
The read tools (`list_profiles`, `list_parameters`, `resolve_profile`, `resolve_from_file`) accept kind `process`, `filament`, or `machine`. Machine presets are Bambu Studio's to write, so `write_profile`, `update_profile`, `diff_profile`, `lint_profile`, `import_profile`, and `remove_profile` accept `process` and `filament`.
