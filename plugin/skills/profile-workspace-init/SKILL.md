...
3. **Probe the stores**:
   - `list_profiles` (kind `process`) with the printer model as search: note the system
     preset names available for this printer — these are the inheritance parents.
   - `resolve_profile` on one of them: read `print_extruder_variant` from the merged
     settings. Its array length and labels are the extruder-variant column shape every
     vector key on this printer uses (e.g. three columns "Direct Drive Standard / High
     Flow / E3D High Flow"). Record length and labels.
   - `list_vendors` for the vendor ids in this installation.
...
- Machine-level values changed in Studio's Printer settings without saving a preset are
  invisible to `resolve_profile` — the resolved system chain then differs from what Studio
  actually uses. Read such keys off the Studio UI or have the user save a machine preset
  first.
- Vendor-shipped presets sometimes inherit another brand's system presets and carry that
  brand's vendor, density, and cost data. Treat them as a reference to harvest deltas
  from, not as an inheritance base, unless verified with `resolve_profile`.
